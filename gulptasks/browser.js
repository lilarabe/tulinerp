...
gulp.task('del-server-code-browser-debug', (cb) => {
...
gulp.task('del-server-code-browser-prod', (cb) => {
...
gulp.task('copy-code-browser-debug', ['stop-cordova', 'del-server-code-browser-debug'], () => {
  const cleanPath = chcpData.browser_local_path_debug;
...
gulp.task('copy-code-browser-prod', ['stop-cordova', 'del-server-code-browser-prod'], () => {
  const cleanPath = chcpData.browser_local_path_prod;
  console.log(`拷贝browser文件 ./platforms/browser/www`);
  console.log(`拷贝到目录 ${cleanPath}`);
  return gulp.src('./platforms/browser/www/**/*')
