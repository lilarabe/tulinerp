 ...
 * 清空 dist debug 目录
 */
gulp.task('clean-android-dir-debug', async () => {
 ...
 * 清空 dist prod 目录
 */
gulp.task('clean-android-dir-prod', async () => {
 ...
 * 清空 serve debug 目录
 */
gulp.task('clean-serve-dir-debug', async () => {
...
//  * 清空 serve prod 目录
//  */
gulp.task('clean-serve-dir-prod', async () => {
...
//  * copy serve debug 目录
//  */
gulp.task('copy-serve-dir-debug', () => {
  const path = chcpData.serve_path_debug;
  const src = chcpData.local_path_debug + '/**/*';
  console.log(`拷贝文件到服务器 debug`);
  console.log(`${src} to ${path}`);
...
//  * copy serve debug 目录
//  */
gulp.task('copy-serve-debug', gulp.series('clean-serve-dir-debug', 'copy-serve-dir-debug', (done) => {
  done();
}));
...
//  * copy serve prod 目录
//  */
gulp.task('copy-serve-prod', gulp.series('clean-serve-dir-prod', 'copy-serve-dir-prod', (done) => {
  done();
}));
...
gulp.task('edit-config-browser', () => {
  return gulp.src('./src/config.js', {
      base: '.'
    })
    .pipe(
      replace(/baseUrl\s*:\s*[\"|\'](\S+)?[\"|\']\s*\,?/i, ($1, $2) => {
        console.log(`修改 config.js baseUrl属性:location.origin`);
        const baseUrl = `Location.origin+'/mapp_port/'`;
        const reg = new RegExp($2);
        return $1.replace(reg, `${baseUrl}`);
      })
    )
    .pipe(gulp.dest('.'));
});
...
// /** 修改 browser config.js 属性 */
gulp.task('edit-config-browser-prod', () => {
  return gulp.src('./src/config.js', {
      base: '.'
    })
    .pipe(
      replace(/baseUrl\s*:\s*[\"|\'](\S+)?[\"|\']\s*\,?/i, ($1, $2) => {
        console.log(`修改 browser config.js baseUrl属性:/mapp_port/`);
        const reg = new RegExp($2);
        return $1.replace(reg, `/mapp_port/`);
      })
    )
    .pipe(gulp.dest('.'));
});

gulp.task('edit-config-wx-prod', () => {
  return gulp.src('./src/config.js', {
      base: '.'
    })
    .pipe(
      replace(/baseUrl\s*:\s*[\"|\'](\S+)?[\"|\']\s*\,?/i, ($1, $2) => {
        console.log(`修改 browser config.js baseUrl属性:https://e.quanzhuyun.com/mapp_port/`);
        const reg = new RegExp($2);
        return $1.replace(reg, `https://e.quanzhuyun.com/mapp_port/`);
      })
    )
    .pipe(gulp.dest('.'));
});

gulp.task('edit-config-wx-debug', () => {
  return gulp.src('./src/config.js', {
      base: '.'
    })
    .pipe(
      replace(/baseUrl\s*:\s*[\"|\'](\S+)?[\"|\']\s*\,?/i, ($1, $2) => {
        console.log(`修改 browser config.js baseUrl属性:https://edemo.quanzhuyun.com/mapp_port/`);
        const reg = new RegExp($2);
        return $1.replace(reg, `https://edemo.quanzhuyun.com/mapp_port/`);
      })
    )
    .pipe(gulp.dest('.'));
});
 ...
 * 设置配置文件完毕
 */
gulp.task('set-config', gulp.series('set-config-xml', 'set-package-json', (done) => {
